...
def generate_metadata_summary(countriesStr):
                            ...
                            for ind,item in enumerate(elem.words):
                                if tag[0].lower() == item:
                                    if tag[1] == elem.grammar_term[ind]:
                                        is_tag_match = True
                                        elem.grammar_word_count[ind] = int(elem.grammar_word_count[ind]) + 1
        ...
        for idx,gt in enumerate(wordelem.grammar_term):
            if gt in NOUN:
                if wordelem not in NOUNTERM:
                    wordelem.word_weight += (wordelem.grammar_word_count[idx] *  weight_dict["noun"])
                    NOUNTERM.append(wordelem)              
            elif gt in ADVERB:
                if wordelem not in ADVERBTERM:
                    wordelem.word_weight += (wordelem.grammar_word_count[idx] *  weight_dict["adverb"])
                    ADVERBTERM.append(wordelem)
            elif gt in ADJECTIVE:
                if wordelem not in ADJECTIVETERM:
                    wordelem.word_weight += (wordelem.grammar_word_count[idx] *  weight_dict["adjective"])
                    ADJECTIVETERM.append(wordelem)
            elif gt in VERB:
                if wordelem not in VERBTERM:
                    wordelem.word_weight += (wordelem.grammar_word_count[idx] *  weight_dict["verb"])
                    VERBTERM.append(wordelem)

    words_object = sorted(words_object, key=lambda x: x.word_weight,reverse = True)
    ...
    ADJECTIVETERM = sorted(ADJECTIVETERM, key=lambda x: x.word_weight,reverse = True)
    ...
    for ind in probable_metadata_index:
        for l in  lines_object:
            if ind >=l.getstartindex() and ind <= l.getendindex():
                l.metadatacount+=1
    lines_object = sorted(lines_object, key=lambda x: x.metadatacount,reverse = True)
